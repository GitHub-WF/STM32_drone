 ...
 * @param {MPU6050_Data_t} *gyroAccel mpu的6轴数据
 * @param {Euler_angle} *EulerAngle 计算后得到的欧拉角
...
void Common_IMU_GetEulerAngle(MPU6050_Data_t *gyroAccel, Euler_angle *eulerAngle, float dt)
{
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(gyroAccel->accel.ax) + squa(gyroAccel->accel.ay) + squa(gyroAccel->accel.az));

    Acc.x = gyroAccel->accel.ax * NormQuat;
    Acc.y = gyroAccel->accel.ay * NormQuat;
    Acc.z = gyroAccel->accel.az * NormQuat;
    // 向量差乘得出的值
    ...
    // 角速度融合加速度积分补偿值
    Gyro.x = gyroAccel->gyro.gx * Gyro_Gr + KpDef * AccGravity.x + GyroIntegError.x; // 弧度制
    Gyro.y = gyroAccel->gyro.gy * Gyro_Gr + KpDef * AccGravity.y + GyroIntegError.y;
    Gyro.z = gyroAccel->gyro.gz * Gyro_Gr + KpDef * AccGravity.z + GyroIntegError.z;
    ...
    float yaw_G = gyroAccel->gyro.gz * Gyro_G; // 将Z轴角速度陀螺仪值 转换为Z角度/秒      Gyro_G陀螺仪初始化量程+-2000度每秒于1 / (65536 / 4000) = 0.03051756*2
    ...
    normAccz = gyroAccel->accel.ax * vecxZ + gyroAccel->accel.ay * vecyZ + gyroAccel->accel.az * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
