...
void Int_MPU6050_Init(void)
{
  ...
  // 9.设置采样频率 500Hz
  Int_Mpu6050_Write_Reg(MPU605_SAMPLE_RATE_DIV, 0x01);

  // 10.配置低通滤波器（建议设置为带宽1或2）
  Int_Mpu6050_Write_Reg(MPU605_DLPH_CONFIG, 0x01);
...
void Int_MPU6050_Get_Gyro(MPU6050_Gyro_t *gyro_data, uint8_t filter_enable)
{
  ...
  {
    log_printf("I2C read error, restart...");
    // I2C 死锁恢复
...
{
  Int_MPU6050_Get_Gyro(&data->gyro, 1);
  Int_MPU6050_Get_Accel(&data->accel, 1);
}
